...
Task("Build")
    ...
    .Does(() => {
       if (DirectoryExists("./src/pong/wwwroot/less"))
        {
            Information("Compiling Less");
            StartProcess("powershell.exe", new ProcessSettings{ Arguments = "-NoProfile -Command lessc ./src/pong/wwwroot/less/site.less ./src/pong/wwwroot/css/site.css"});
        }

        if (DirectoryExists("./src/pong/wwwroot/ts"))
        {
            Information("Compiling TypeScript");
            StartProcess("powershell.exe", new ProcessSettings{ Arguments = "-NoProfile -Command tsc --outFile ./src/pong/wwwroot/js/site.js -t ES5 ./src/pong/wwwroot/ts/site.ts"});
        }
    });
