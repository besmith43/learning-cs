...
namespace fixingCakeTool.cmd
{
    public class Options
    {
        ...
        public Options()
        {
            helpFlag = false;
            versionFlag = false;
            versionText = $"fixingCakeTool { Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyFileVersionAttribute>().Version }";
            verbose = false;
            StringBuilder helpTextBuilder = new StringBuilder();
            helpTextBuilder.AppendLine($"{ versionText }");
            helpTextBuilder.AppendLine("Usage:  fixingCakeTool [OPTION]  ");
